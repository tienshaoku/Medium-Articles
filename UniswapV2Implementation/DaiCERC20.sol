...
abstract contract CToken is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
     ...
     */
    function checkTransferIn(address from, uint amount) internal virtual view returns (Error);
...
contract CErc20 is CToken {
     ...
     *      whether or not `from` has a balance of at least `amount`. Does NOT do a transfer.
     */
    function checkTransferIn(address from, uint amount) internal override view returns (Error) {
        EIP20Interface token = EIP20Interface(underlying);
